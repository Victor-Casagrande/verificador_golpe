/**
 * Aplicação Express do Sentinela — montagem de middlewares, rotas e endpoints
 * públicos (status, índice HATEOAS, Swagger).
 */
...
app.get("/api/status", async (req, res) => {
  ...
  res.status(200).json({
    ...
    mensagem: dbHealth.ok
      ? "API do Sentinela operando normalmente."
...
app.get("/", (req, res) => {
  ...
  res.status(200).json({
    sucesso: true,
    mensagem: "Bem-vindo à API Sentinela",
    versao: "1.0.0",
    links: {
      self: {
        href: `${baseUrl}/`,
