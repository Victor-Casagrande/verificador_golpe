...
(async () => {
  ...
  await page.evaluateOnNewDocument(() => {
    window.chrome = {
      ...
      tabs: {
        create: (opts) => { window.mockTabsCreateCalled = opts; }
      }
    };
  });
  ...
  const dashboardHiddenA = await page.$eval('#dashboard-view', el => el.classList.contains('hidden'));
  ...
  const dashboardHiddenB = await page.$eval('#dashboard-view', el => el.classList.contains('hidden'));
  
  console.log("Login visível?", !loginHiddenB);
  console.log("Dashboard visível?", !dashboardHiddenB);
  
  const historyItems = await page.$$eval('#history-list li', lis => lis.map(li => li.outerHTML));
