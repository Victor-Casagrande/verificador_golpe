 ...
 * Uso: node scripts/test-urls-local.js [--base-url=http://localhost:3000]
 *
...
const analyzeUrl = async (url, accessibilityReport = []) => {
  const res = await fetch(`${baseUrl}/urls/analyze`, {
    ...
    body: JSON.stringify({ url, accessibility_report: accessibilityReport })
  });
...
const main = async () => {
  ...
  try {
    ...
    if (!health.ok) {
      console.error('API não respondeu ao health check. Suba o servidor antes de rodar este script.');
      process.exit(1);
    }
  } catch {
    console.error(`Não foi possível conectar em ${baseUrl}. Execute: npm run dev`);
    process.exit(1);
  }

  await runGroup('URLs seguras (esperado: Seguro)', fixtures.safe);
  await runGroup('URLs suspeitas — heurística local', fixtures.heuristic_suspicious);
  await runGroup('URLs inválidas', fixtures.invalid);
  await runGroup('Com relatório de acessibilidade (fixture)', [fixtures.safe[0]], {
